...
module.exports = {
    ...
    create: async (req, res) =>{
        ...
        */
        ...
        if (!req.body?.price) {
            const dataPizza = await Pizza.findOne({ _id: req.body.pizzaId }, { _id: 0, price: 1 } ) // 0 ve 1 yerine sırasıyla false ve true da yazılabilirdi
        ...
        res.status(201).send({
            error: false,
            details: res.getModelListDetails(Order),
            data
        })
    ...
    update: async (req, res) =>{
        ...
        */
           ...
           req.body.quantity = req.body?.quantity || 1 // default: 1
           if (!req.body?.price) {
               const dataOrder = await Order.findOne({ _id: req.para.id }, { _id: 0, price: 1 } ) // 0 ve 1 yerine sırasıyla false ve true da yazılabilirdi
